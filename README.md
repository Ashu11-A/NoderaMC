...
|---|---|---|---|
| `core` | domain types, JDK-only crypto (Ed25519/SHA-256 + Task 23 AES-GCM/PBKDF2), canonical encoding (frozen wire/hash contract) + `ServerAuthorityCertificate` (Task 11) + `CommitteeChangeCertificate` (Task 9) + entity-lane foundation `FixedVec3`/`NetworkEntityId`/`PersistedEntityState` + item actions/events (Task 12a) + `WorldRole` + tags 92/93 (Task 33) | 164 | ✅ |
