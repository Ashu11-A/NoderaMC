...
**Overall system completion: `97.8%`**
`███████████████████░`
...
| `paper-plugin` | (server task 1) `nodera-endpoint.jar` — the Paper/Folia endpoint plugin: platform detection by the presence of the regionised scheduler (a fork renames itself; the scheduler is the load-bearing difference), and the **ALIGN-1 preflight** that refuses to enable where a Nodera region would straddle two Folia sections. Green on real Paper 1.21.1 and Folia (`e2e-endpoint.sh`, `e2e-folia.sh`, `e2e-plugins.sh`), and `nodera-endpoint.yml` is parsed and enforced at enable; hosts no peer yet (server task 2) | 14 | 🚧 |
