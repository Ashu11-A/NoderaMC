...
| `rust/nodera-telemetry` | (telemetry 1–2) telemetry ingest service — the consent gate, the event registry that no free-text value can enter, forward-secret rotating pseudonymisation (per-period OS-CSPRNG key, memory-only, wiped on rotation), coarse country/ASN geolocation with the address discarded, per-source quotas, and a rotating NDJSON spool for the Big Data plane in `docker/telemetry/`. Carries **no authority**: nothing in the network reads it | 93 | ✅ |
| `rust/nodera-app` | (Task 32 · app 5) Tauri companion app — always-on headless-peer supervisor (Option B: bundled Java peer) + loopback control endpoint (mod presence gate) + system tray + autostart + React dashboard (chunks/GB/peers/world). Workspace-EXCLUDED (Tauri native deps); built separately | 187 | 🚧 |
