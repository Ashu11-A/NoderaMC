...
> monorepo restructure ([`MONOREPO.md`](docs/MONOREPO.md)), a standalone `nodera-tracker` service, and
> a `nodera-rendezvous` relay for NAT reach — all ⬜, so the overall % drops from 68% without any
> regression. The embedded Java tracker and the `transport-libp2p` plan they supersede are
> ledgered in [`LEGACY.md`](docs/LEGACY.md).
