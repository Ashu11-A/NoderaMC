...
| 9 | Peer-runtime + event-sourced storage | 5 | `#9` | 🚧 (`peer-runtime` membership + gateway migration; **event-sourced `WorldStore` seam + in-memory impl** with certified-chain replay + forward sync; **RocksDB archival tier shipped** — `storage-rocksdb` crash-consistent durable store + `FsContentStore` + forced-kill recovery IT; new-peer live sync + committee-change certificates deferred) |
