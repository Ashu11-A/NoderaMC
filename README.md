...
**Tests:** `744 passing · 0 failing · 0 skipped` (+**144 Rust**) (adds **Task 31 GUI redesign + Task 32
...
| `diagnostics` | Minecraft-free telemetry: TrafficMeter/RateWindow/MessageCounters, integer-EMA TickSkewMeter/TpsMeter, TelemetrySnapshot, ZoneClassifier, Panel/Row/Cell view model (Tasks 18/25) + `TorrentWorldListView` (Task 26) + Task 31 GUI models `PublicWorldBadgeView`/`PieceMapView`/`TrackerStatusView`/`RendezvousStatusView` | 75 | ✅ |
...
Task 30 retired the central NeoForge dedicated server. One script builds both toolchains and runs
the two **untrusted** infrastructure services — `nodera-tracker` (peers locate worlds) and
`nodera-rendezvous` (NAT hole-punch + relay). The world itself is hosted by a player's own client:

```bash
scripts/dev.sh                   # build Rust + mod, collect artifacts, run tracker + rendezvous
...
use `--install-mod`), launch the client, open a world, and press **"Share to Nodera"** in the pause
menu (the analogue of "Open to LAN") to broadcast it to the network — with an optional password.
...
| 32 | **Companion app**: required Tauri headless-peer daemon + mod presence gate (always-on node; hosts no longer need to stay online) | 6 | — | 🚧 (Option B locked — Tauri supervises a bundled headless Java peer; `rust/nodera-app` scaffold (backend + React dashboard + tray + autostart, workspace-excluded), mod-side `CompanionGate`/`CompanionClient`/`CompanionProtocol` presence gate green (+8); enforcement off until the app ships; headless-peer jar + live metrics + cross-machine continuity deferred — L-41 RETIRING, new L-47/L-48) |
