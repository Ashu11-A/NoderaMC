...
| L-3 | Gravity blocks + fire excluded. **Instant-settle gravity + bounded fire landed 2026-07-23:** `GravityRules` — sand + gravel (palette 64) settle INSTANTLY down their column (no falling entity; the landed result commits in the acting batch — parity envelope: vanilla animates the fall over ticks, the validated lane commits the outcome, client interpolation reconciles visually); place/break arms hook `onPlaced`/`onVacated`, a broken support drops the whole contiguous stack bottom-first, and falls are strictly vertical so gravity needs NO border signalling. Fire (palette 65) rides `RandomTickRules`: one selection in three extinguishes, the rest attempt a 3×3×3 spread that BURNS flammable targets (planks/logs become fire) — growth is bounded by fuel, every blaze dies out; extinguished flames settle whatever rested on them. `GravityFireRulesTest` (5): instant landing, column drop (unit + engine path), fuel-only spread + guaranteed burnout, 300-tick blaze replica-identical roots. Remaining: fluid-gravity coupling (sand through water), piston-move settle hook, live evidence | T14 | Sand/gravel fall + fire spread validated; parity envelope vs vanilla documented | OPEN |
| L-4 | No deterministic lighting (light-dependent mechanics can't validate). **Deterministic lighting landed 2026-07-23:** `LightField` computes sky + block light as a PURE FUNCTION of committed block state — light is DERIVED from the root, not stored beside it, so replicas agree on every light-dependent decision by construction and there is nothing to drift (the design deliberately trades the audited 'hash the nibbles' plan for the stronger property: a pure function of hashed state needs no separate commitment). Sky: open columns read 15, vanilla down-beam asymmetry (straight down from 15 stays 15, all else decays 1), lateral leak under overhangs; block: emitters (fire/lava 15, lit redstone torch 7) BFS with 1-decay per hop that goes AROUND walls; opacity is a solid-cube whitelist so non-solid components pass light and emitters shine out of their own cell; bounded ±15 sampling box is EXACT (15 is the maximum reach), with fast paths for opaque cells and open sky. **Consumers wired:** grass random ticks now gate on light (dies < 4 at the cell above — the smother rule became the light rule; spread needs ≥ 9 over the receiving dirt) — the L-8 spawning lane reads the same API. `LightFieldTest` (6, first-run green): golden sky/overhang/roof values, per-hop glow decay + around-walls, torch 7, purity; grass soak regressions unchanged. Remaining: mob-spawn light gate consumption (L-8/T15), halo light carriage at borders, live evidence | T14 | Engine skylight/blocklight matches committed roots; crops/spawning read engine light | OPEN |
| L-5 | Observer + quasi-connectivity missing; redstone palette v2 semantic gaps. **Observer + QC landed 2026-07-23:** observers (palette 66–73, facing = the WATCHED direction; OFF placeable, ON minted) pulse 15 out their BACK for exactly one tick — rise scheduled +1 after the observed change, fall +1 after the rise, BOTH edges in the hashed queue (the pulse is consensus phase); `observersOnChange` hooks every mutation family (place/break arms, redstone recompute's changed set, fluid updates, gravity settles via the arms). Quasi-connectivity: `pistonInputPowered` also reads the cell ABOVE the piston as-if-a-block-sat-there — and because pistons only re-evaluate on received updates, classic BUD staleness EMERGES from the scheduling model instead of being simulated. `COMPATIBILITY.md` §4 rewritten to the current truth (validated lane owns redstone in delegated regions; vanilla ticks cancelled at source; group demotion on vanilla-lane crossings). `ObserverQcTest` (5, first-run green): rise timing + back-wire 15, self-falling pulse + replica determinism, back-only emission (side wire reads the 14 hop, never 15), QC extension through the above cell, ON-state mint protection. Remaining: observer watching piston-moved cells (migration lane), live evidence | T14 | Palette v3 ships observer + QC; `COMPATIBILITY.md` gap note deleted | OPEN |
| L-6 | Daylight/time-coupled blocks excluded (~~time not in validated context~~ **context clause done 2026-07-23:** `RegionExecutionContext` carries `committedWorldTime` — a member-agreed, root-determining input (compat constructor pins 0 for time-free rule sets; `committedWorldTimeIsCarriedAndDefaultsToZero`); remaining: the daylight-sensor rule with the T14 palette) | T14 | Committed world-time in `RegionExecutionContext`; daylight sensor validated | OPEN |
| L-7 | Mobs are server-authoritative ghosts; AI not validated. **Deterministic ghost-mob AI landed 2026-07-24 (L-7 core):** `MobAiRules` on the engine tick hook is the first per-species retirement step — `GHOST` zombies now MOVE from the validated root, not from the server's mob. Every `AI_INTERVAL_TICKS` (10) each ghost draws ONE decision from the per-tick `DeterministicRandom` in canonical id order: idle (3/8) or a one-block horizontal step in one of four directions (the decision count is fixed regardless of branch, so the rng stream stays aligned across replicas); a step lands only on a WALKABLE cell (solid floor + two air headroom, ±1 climb/drop, block-centred result) — the root always holds a legal stance, and region borders fail closed. Ghosts past their `despawnTick` horizon are removed every tick (the population breathes exactly like vanilla's despawn cycle, deterministically). `MobAiRulesTest` (3, first-run green): 2400-tick dark-shelter soak — EVERY ghost ends on a walkable cell with `ageTicks>0` AND replica-identical roots; direct-state wander eventually steps; the despawn horizon fires exactly on time. `EntityRuleSet.tick` drives `MobAiRules.tick` after `SpawnRules.tick`. **First species' behavior now originates in the validated engine, not vanilla** — the ghost-share target becomes reachable once per-species rates + mirroring arrive. Remaining: targeting/pathfinding/combat, per-species retirement + the ghost-mirroring/`mobCapture` default flip that rides it (L-24), live evidence | T15 | Per-species retirement: ghost-share = 0 for every shipped species in soak | OPEN |
| L-8 | Mob spawning cycles vanilla-only. **Deterministic spawn cycles landed 2026-07-23 (T15 opener):** `SpawnRules` on the engine tick hook — every `SPAWN_INTERVAL_TICKS` (20) one attempt draws a random owned column from the per-tick `DeterministicRandom`, collects the column's ACTUAL standing cells (solid floor + two air headroom; roof and cave floor are both candidates), draws among them, and spawns only when combined `LightField` light < 8 and the region's ghost population is under `MOB_CAP` (8) — interval × cap is the documented vanilla rate envelope. Spawns enter the ROOT as `EntityKind.GHOST` zombies with deterministic ids (`NetworkEntityId.allocate` in a dedicated spawn seq domain) and a 6000-tick despawn horizon; the live lane mirrors them like captured mobs. `SpawnRulesTest`: 2000-tick dark-shelter soak — populated ≤ cap, EVERY spawn inside the shelter (never the lit roof), replica-identical roots; lit platform spawns zero; empty world spawns zero. Remaining: passive species + per-species rates, mod-side ghost mirroring of engine spawns, vanilla spawn suppression mixin, live evidence | T15 | Deterministic spawn cycles (engine light + seeded RNG) match vanilla rate envelope | OPEN |
| L-9 | Projectiles, TNT, rails/minecarts not validated. **Deterministic TNT landed 2026-07-24 (L-9 warmup rung):** a primed-TNT entity is now engine-owned state — `EntityKind.TNT` (ordinal 2; ITEM/GHOST wire bytes unchanged, decode accepts the new ordinal, `EntityLaneTypesTest` round-trips all kinds generically). A TNT entity's `despawnTick` IS its detonation tick; on detonation a blast-local `DeterministicRandom` seeded from `(BLAST_DOMAIN, entityId, detonateTick, pos)` drives a per-cell destruction pass over the cube `[-R,R]³` in canonical `(dy,dx,dz)` order — a cell within Euclidean cutoff R is destroyed when `blastRng.nextInt(R²) < R² - dist²` (destruction probability falls off linearly with squared distance; centre always clears, edge never does ⇒ roughly spherical crater). Border cells are skipped (fail-closed — cross-region blast via the T13 group-migration lane is a later increment, `@Invariant(11)`); one draw per in-range non-air cell keeps the rng stream aligned across replicas. **Chain ignition:** a detonation shortens the fuse of every other TNT within the radius to `detonateTick+1` (vanilla ignites primed TNT in a blast — replicated without a captured mob). `TntRulesTest` (3, first-run green): seeded crater + replica-identical roots + centre-clears + cutoff-respected + entity-removed; fuse fires exactly on time (no early blast); chained TNT ignites + clears in deterministic sequence. `EntityRuleSet.tick` drives `TntRules.tick` after `MobAiRules.tick`. Remaining: projectile ballistics (arrows/pearls/snowballs, Q32.32 + integer raycast hit detection), ~~entity knockback impulse~~ **(knockback landed: per-axis outward impulse, magnitude `(R²−dist²)×KNOCKBACK_BASE`, applied to kinematic non-TNT entities; mobs not yet kinematic)**, blast-destruction hooks into redstone/gravity/observer recompute, cross-region blast via migration, cart kinematics + powered rails (read T13 redstone), the player action that ignites a placed TNT block, live evidence. **Deterministic projectiles landed 2026-07-24 (L-9 increment 2):** `EntityKind.PROJECTILE` (ordinal 3) + `ProjectileRules` — pure Q32.32 ballistics integrated in a FIXED order (`vy=(vy-gravity)*drag; vx=vx*drag; vz=vz*drag; pos+=vel`; gravity-before-drag pinned, drag 0.99 / gravity 0.05 pre-baked literals — no float in hashed state). Hit detection: an opaque destination block stops the shot dead (velocity zeroed, held one tick short of the wall — "arrow embedded"); transparent cells pass; border crossings transfer the entity to the neighbour region like items; stuck shots rest until their `despawnTick` horizon. `ProjectileRulesTest` (4, first-run green): arc + replica-identical trajectory roots + gravity-bent-down, gravity-acts-before-drag direct step, stick-to-opaque-wall + no-pass-through + replica roots, lifetime-horizon despawn. Remaining: cart kinematics + powered rails (read T13 redstone), entity-hit detection (arrow strikes a mob) — **landed: the march now stops on a non-projectile entity within a half-block radius (arrow embedded in a mob); damage application is L-13**, voxel-DDa face-snap + water slow-down, the player fire action, live evidence. **Deterministic minecarts landed 2026-07-24 (L-9 increment 3):** `EntityKind.MINECART` (ordinal 4) + palette `RAIL`(74)/`POWERED_RAIL`(75) + `RailRules` — pure Q32.32 kinematics on a rail graph. A cart holds an axis-aligned horizontal velocity and FOLLOWS the track by inferring its heading from neighbour connectivity (no rail-shape states): continue in the current direction when the forward cell is rail, else turn onto the unique perpendicular rail (canonical N,E,S,W tie-break; reverse never chosen), else stop at the dead-end; the cross-axis snaps to the rail centerline each tick. Speed: a `POWERED_RAIL` pins it to `MAX_SPEED` (boost); a plain rail bleeds it by `FRICTION` (0.998); a cart at rest stays at rest (needs a push — never auto-reverses). Border crossings transfer the cart to the neighbour region. Constants pre-baked Q32.32 literals — no float in hashed state. `RailRulesTest` (3, first-run green): closed powered loop circulates 200 ticks + replica-identical lap roots + cart still on-rail-and-moving; dead-end stops the cart dead + rests (no reverse) + replica roots; powered rail holds `MAX_SPEED` every tick + replica roots. **All three L-9 species cores (TNT/projectile/cart) now engine-owned.** Remaining: ~~redstone-gated powered rails (read T13 power)~~ **(landed: `RedstoneRules.cellReceivingPower` public wrapper over `cellWouldBePowered`; a `POWERED_RAIL` boosts only when its cell receives redstone power — unpowered, it coasts like a plain rail)**, rail slopes/ascent gravity, entity-hit detection, voxel-DDA, player place/fire/ride actions, cross-region blast via migration, live evidence | T15 | Arrow/pearl/TNT/cart determinism fixtures green on 3 replicas | OPEN |
...
| L-20 | Genesis is a single-signer trust root (server self-certifies). **Mechanism landed 2026-07-23:** `GenesisRecertification` (type tag 104, Rust-mirrored) — the founding peer set endorses the genesis root with a strict-majority quorum of distinct founder signatures over (root ‖ complete founding set), so no subset can shrink the set to lower the bar; outsider/duplicate/forged approvals never count (`GenesisRecertificationTest`, incl. canonical round-trip). Remaining: the share-flow wiring that collects founder approvals as peers join | T16 | Multi-party genesis re-certification signed by founding peer set | RETIRING |
...
| L-39 | World content is plaintext on the P2P net; any connected peer can read any chunk; no per-world encryption. **Throttle half done 2026-07-23:** `JoinAttemptThrottle` (peer/distribution) — per-(world, peer) failed password attempts open exponentially growing lockouts (3 free, 5 s base, doubling, bounded at 1 h — never permanent), refused attempts never touch the KDF (no work amplification), success clears, peers independent (`JoinAttemptThrottleTest`). Remaining: the worker SEED/HOST path deriving a `ContentKey` and emitting `EncryptedRegion` pieces + the join-side password prompt (GUI) | T23 | AES-GCM-256 content encryption under Argon2id(password)-derived key; seeders store ciphertext, verify by hash; join requires password; ciphertext-integrity + wrong-password + nonce-uniqueness tests green | RETIRING |
