...
|---|---|---|---|---|
| L-1 | Random ticks suppressed in delegated regions (no grass/fire/crop ticking there). **Engine-owned random ticks landed 2026-07-23 (T14 opener):** `RandomTickRules` on the engine tick hook — vanilla-shaped selection (3 draws per eligible 16³ section per tick from the per-tick `DeterministicRandom` reserved since Task 3), canonical column order, state-derived section eligibility (ineligible sections skip WITHOUT consuming randomness, so replicas skip identically); grass MVP semantics (smothered grass dies to dirt; one 3×3×3 spread attempt per selection onto dirt-with-air-above; light gates arrive with L-4, cross-border spread waits for the halo lane). `RandomTickRulesTest`: direct semantics pins + the acceptance core — 200-tick soak over a grass/dirt checkerboard, IDENTICAL roots on 3 replicas with the lane actively spreading, and nothing-tickable worlds keep their delta untouched. Remaining: fire/crops (palette additions ride L-3/L-4), live random-tick suppression mixin + farm soak with counter-zero (live evidence) | T14 | Farm soak: engine-owned random ticks, identical roots on 3 replicas, suppression counter deleted | RETIRING |
| L-2 | Fluids excluded from validated lane. **Finite deterministic fluids landed 2026-07-23:** `FluidRules` is a per-cell automaton riding the T13 hashed scheduled-tick queue — pending fluid updates are consensus state (they survive delta boundaries in the `RegionDelta` v4 lists like piston motion). Level encoded IN the block id (palette 52–63: water source + flows 1–7, lava source + flows 1–3; sources placeable, flows minted); desired state is a pure neighborhood function (falling first — a hanging column never pyramids: horizontal contribution requires the neighbor to SIT ON SOLID; support loss decays flows to air, so breaking the source drains the network); vanilla cadence (water 5, lava 30) and reach (water 7, lava 3); water outcompetes lava deterministically (interactions later); fluids don't block pistons (the push destroys them); spread across the border emits `BorderSignal.Kind.FLUID` — no halo writes, pinned. `FluidRulesTest` (6, full engine path, first-run green): level-per-hop + finite reach + replica-identical roots, source-break drain, fall-before-spread + pooling, lava reach 3, mint protection, border signal. Remaining: cross-region spread consumption (halo/migration lane), fluid interactions (obsidian/cobble), live evidence | T14 | Water/lava spread deterministic across replicas, incl. cross-region via T13 migration | RETIRING |
| L-7 | Mobs are server-authoritative ghosts; AI not validated. **Deterministic ghost-mob AI landed 2026-07-24 (L-7 core):** `MobAiRules` on the engine tick hook is the first per-species retirement step — `GHOST` zombies now MOVE from the validated root, not from the server's mob. Every `AI_INTERVAL_TICKS` (10) each ghost draws ONE decision from the per-tick `DeterministicRandom` in canonical id order: idle (3/8) or a one-block horizontal step in one of four directions (the decision count is fixed regardless of branch, so the rng stream stays aligned across replicas); a step lands only on a WALKABLE cell (solid floor + two air headroom, ±1 climb/drop, block-centred result) — the root always holds a legal stance, and region borders fail closed. Ghosts past their `despawnTick` horizon are removed every tick (the population breathes exactly like vanilla's despawn cycle, deterministically). `MobAiRulesTest` (3, first-run green): 2400-tick dark-shelter soak — EVERY ghost ends on a walkable cell with `ageTicks>0` AND replica-identical roots; direct-state wander eventually steps; the despawn horizon fires exactly on time. `EntityRuleSet.tick` drives `MobAiRules.tick` after `SpawnRules.tick`. **First species' behavior now originates in the validated engine, not vanilla** — the ghost-share target becomes reachable once per-species rates + mirroring arrive. Remaining: targeting/pathfinding/combat, per-species retirement + the ghost-mirroring/`mobCapture` default flip that rides it (L-24), live evidence | T15 | Per-species retirement: ghost-share = 0 for every shipped species in soak | RETIRING |
| L-8 | Mob spawning cycles vanilla-only. **Deterministic spawn cycles landed 2026-07-23 (T15 opener):** `SpawnRules` on the engine tick hook — every `SPAWN_INTERVAL_TICKS` (20) one attempt draws a random owned column from the per-tick `DeterministicRandom`, collects the column's ACTUAL standing cells (solid floor + two air headroom; roof and cave floor are both candidates), draws among them, and spawns only when combined `LightField` light < 8 and the region's ghost population is under `MOB_CAP` (8) — interval × cap is the documented vanilla rate envelope. Spawns enter the ROOT as `EntityKind.GHOST` zombies with deterministic ids (`NetworkEntityId.allocate` in a dedicated spawn seq domain) and a 6000-tick despawn horizon; the live lane mirrors them like captured mobs. `SpawnRulesTest`: 2000-tick dark-shelter soak — populated ≤ cap, EVERY spawn inside the shelter (never the lit roof), replica-identical roots; lit platform spawns zero; empty world spawns zero. Remaining: passive species + per-species rates, mod-side ghost mirroring of engine spawns, vanilla spawn suppression mixin, live evidence | T15 | Deterministic spawn cycles (engine light + seeded RNG) match vanilla rate envelope | RETIRING |
| L-10 | Comparator, hoppers, note block outside every lane (need containers). **Container state model landed 2026-07-24 (T16 container-lane increment 1):** container contents are REGION STATE — `ContainerEntry` (type tag 105, Rust-mirrored: pos + ordered `[u32 itemStackId][u8 count]` slots, empty = (0,0), ≤ 54) lives in the hashed root at snapshot body v4 and rides deltas with REPLACE semantics at delta body v5 (an emptied chest clears on the applier; pre-container transitions keep their exact bytes). `MutableRegionState` carries the canonical container table (`container`/`putContainer`/`removeContainer`), `SnapshotDeltaApplier` mirrors the version choice byte-exactly. `ContainerStateRootTest` (5). **Increment 2 landed same day:** `CHEST` (palette 77) + `ContainerAction` (tag 29, live: pos + origin + mode + slot + item + count) — the committee validates container kind, slot bounds, reach ≤ 4/axis, and the slot's CURRENT contents (no conjuring into an occupied slot, no overdraft, no wrong-item withdraw — `BAD_PREVIOUS_STATE`); withdraw credits the player through the same replay-safe `InventoryCredit` lane as pickup; breaking a chest spills its committed contents as validated ITEM entities (`ContainerRules`, `ContainerRulesTest` 5). **Increment 3 landed same day:** `HOPPER` (palette 78) — a placed hopper self-schedules an 8-tick cycle on the HASHED queue (`ContainerRules.hopperTick`: push one item down, pull one from above, reschedule; a broken hopper leaves a stale no-op entry), so a chest→hopper→chest column drains deterministically with replica-identical roots (`ContainerRulesTest.hopperDrains…`). **Increment 4 landed same day — the engine core is COMPLETE:** comparators (palette 79–82, facing = output) emit the fill level of the container BEHIND them — a state-dependent directional source special-cased in the BFS seeding like the daylight sensor, with the vanilla signal shape (1 + 14·total/(slots·64), integer math) — and container mutations are REACTIVE: deposit/withdraw/hopper cycles re-settle adjacent comparator networks at the source (`RedstoneRules.onContainerChanged` — the L-6 static-sensor gotcha fixed here). `NOTE_BLOCK` (83) joins the palette (sound is client presentation, not consensus state). Delegability is palette-driven, so regions with chests/hoppers/comparators are delegable NOW. `ContainerRulesTest` (7). Remaining: chest GUI + live evidence (mod-side) | T16 | Container lane ships; palette completes; regions with them delegable | RETIRING |
| L-11 | Player inventory outside region root (pickup = one-way credit). **Core landed 2026-07-24:** `EntityKind.PLAYER` — the player’s root presence whose payload carries owner `NodeId` + the 36-slot inventory (`PlayerRules`, ItemSlot encoding shared with containers). Item pickups and container withdrawals now land in the VALIDATED root inventory when the actor’s player entity is registered (the one-way `InventoryCredit` survives ONLY as the migration stopgap — no player entity or full inventory: never lost, never duped either way). A region/dimension hand-off moves the whole inventory through the same dupe-proof joint-certificate transfer pipeline as any entity — `PlayerInventoryTest` (3): root-inventory pickup (credit lane empty), stopgap pinning, portal hand-off carrying the loaded inventory with the source cleared exactly once. Remaining: mod-side player-entity registration on the live lane, deposit debit (needs the inventory to be the ONLY source of truth — flips with the vanilla-capture retirement), live evidence | T16 | Inventory state validated; dupe-proof under cross-region transfer tests | RETIRING |
| L-12 | Player movement server-authoritative. **Engine core landed 2026-07-24:** `MovePlayerAction` (tag 106, Rust-mirrored) — the client PROPOSES a step, the committee decides legality (`MovementRules`): per-axis speed envelope (≤ 1 block/action), destination passability (feet + head not opaque), and a border step becomes the dupe-proof cross-region transfer carrying the whole player payload (inventory + health) — validated cross-region movement with no special case. Teleports, speed hacks, and wall clips die in validation on every honest replica (`MovementRulesTest`, 3). Remaining: the client prediction/rollback overlay (rides L-16 LocalReplicaView live wiring), mod-side capture of vanilla movement into `MovePlayerAction`s, live evidence | T16 | Optimistic validated movement with rollback; cheat-movement rejected by committee | RETIRING |
| L-13 | Combat / health / XP not validated. **PvE core landed 2026-07-24 (T16 opener):** `EntityKind.MOB` is the engine-owned validated mob — vitals live IN the root as the canonical `[u16 health][u16 maxHealth]` payload (`MobCombatRules`), spawns enter as MOB with full vitals, `MobAiRules` drives them like ghosts, and every damage source routes through the single `MobCombatRules.damage` mutation point: arrow strikes (flat 5) and blast proximity (linear falloff, integer math); health ≤ 0 removes the entity — death is committed, replica-identical state (`MobCombatTest`, 8). GHOSTs are never wounded by the engine (server-authoritative vitals). **Player-dealt melee landed same day:** `AttackEntityAction` (tag 28, now live) carries target + attacker origin; the committee validates reach (≤ 4 blocks/axis) and target kind, and the DAMAGE is the rule-set constant `MELEE_DAMAGE` — a modified client cannot hit harder or farther than the rules allow. **PvP half landed same day (941e26b):** `PlayerState` carries committed health; `AttackEntityAction` accepts PLAYER targets; `MobCombatRules.damage` stays the single mutation point so arrows/blasts wound players automatically; death removes the root presence AND spills the committed inventory as validated ITEMs (nothing dupes, nothing vanishes; respawn = live-lane re-registration) — `PlayerInventoryTest` PvP cases. Remaining: XP, mob drops, live evidence | T16 | PvP + PvE damage validated; state in root; `@Invariant(10)` extended test | RETIRING |
| L-14 | Portals, dimensions, commands outside validated path. **Portal half landed 2026-07-24:** cross-dimension travel IS generalized region transfer — `RegionId` always carried its `DimensionKey` and `EntityTransferIntent` is dimension-agnostic, so `PortalRules` just emits a transfer intent whose target region lives in the nether (`NETHER_PORTAL` palette 84; vanilla 8:1 coordinate scale in pure fixed-point; GHOSTs never engine-portal). Same prepare/accept/commit pipeline, same joint certificates, no new protocol. `PortalRulesTest` (2). Remaining: the deterministic command subset, nether-side world/genesis provisioning, live evidence | T16 | Cross-dimension = generalized region transfer; deterministic command subset | OPEN |
| L-15 | Regions outside generated terrain non-delegable. **Engine worldgen core landed 2026-07-24:** `WorldGenRules` — terrain is a PURE FUNCTION of (worldSeed, position) built entirely on `StableHash` integer math (no doubles, no noise libraries — A-5 by construction): hash-derived lattice offsets, integer bilinear interpolation, stone/dirt/grass layering; `GENERATOR_FINGERPRINT` pins the shape (any change is a fingerprint bump, never a silent re-pin). `WorldGenRulesTest` (3): purity + seed-sensitivity, golden column hashes, cross-border continuity (no interpolation cliffs). **Reason DELETED same day:** `DelegabilityPolicy.Reason.OUTSIDE_GENERATED_TERRAIN` and its `terrainGenerated` input are gone — with terrain derivable from the seed alone, "outside generated terrain" stopped being a refusal class (both exit halves met). Remaining: coordinator generate-on-demand call site on the live lane, caves/biomes/features, live evidence | T16 | Deterministic world-gen-from-seed in engine; `OUTSIDE_GENERATED_TERRAIN` reason deleted | RETIRING |
| L-16 | Committed effects appear 1–2 ticks late (ghost/pending render) **Commit-feed wiring landed 2026-07-24 (d69fc36):** `WorkerValidationService.onCommit` streams every committed snapshot+root to one observer, and `ClientValidationLane` now constructs a `LocalReplicaView` and feeds it on every applied commit (`replicaView()` accessor; observer asserted in `ActionForwardIT`). Remaining: the renderer bind to `render()` + capture-point `predict()` calls — the GUI half. | T16 | Client prediction + rollback: local apply instant, reconciliation invisible in normal play (interim: T6 ghost render) | OPEN |
...
| L-49 | Task 33 landed the world-identity/authorship + P2P permission model + live worker telemetry headlessly, but several enforcement/UX halves ride the live NeoForge/worker mesh: the singleplayer world-list per-row "server-like" player count needs the first mixin (`WorldSelectionListEntryMixin`) in a GUI env; live chunk/region validation + revalidation needs the `committee`/`coordinator` stack wired over the worker's `PeerRuntime` (the headless `CommitteeMvpIT` pipeline, now live); ~~`BANNED` enforced at `JOIN`~~ (**done 2026-07-23:** `PeerRuntime.JoinAdmission` gates both `onPeerJoin` and gossip ingest; the hosted world installs `WorldPermissions::canJoin` at share time (`NoderaHost.hostedPermissions`); `BannedPeerAdmissionTest` proves refusal at join AND via gossip — grant gossip/announce remains with the live mesh half); password change re-encryption must propagate over the network (Task 23 live); the worker must seed extracted region pieces | T33 | `runClient`: a shared world shows a live player count on its row; a committee re-executes + commits a region delta over the worker mesh; a BANNED peer is refused at join; a re-keyed world's new ciphertext replaces the old across seeders | OPEN |
| L-50 | Task 12's live activation now bootstraps itself and is **proven live** (2026-07-22): `EntityLaneBootstrap` derives the interim seed-based genesis + all-AIR flat snapshots + FOV-plan epoch-1 leases; `NoderaHost.activateEntityLaneFromWorld` wires it behind `entity.laneAutoActivate` (default off), triggered at share time or on first login (`ServerBootstrap`, crash-contained); startup **compensates** dangling RESERVED actions (`DurableActionJournal.abortPending`). Live evidence: dedicated `runServer` + scripted quick-play `runClientJoin` → `entity lane live on 12 region(s) around Dev`, P2P mesh formed, zero errors. Activation is **asynchronous** (dedicated `nodera-entity-lane-boot` thread, stop-race guarded), the entity lane opens against the **certified genesis** (30c), and the **first scripted gameplay assertion passed live (2026-07-22)**: RCON-driven `summon` on a running server+joining-client pair → capture → solo-committee commit → `/nodera entities` panel (now fed by `LiveEntityControlProvider` — the entities half of L-31 exits) showing **239 validated/ghost entities across 12 delegated regions with versions advancing every flush**. Two live findings fixed en route: entities whose chunks predate activation never fired capture events (activation now **sweeps** bound regions), and with `mobCapture` off every spawn-area mob gracefully revoked its region within ticks (acceptance-#3 behavior, confirmed live — the scripted scenario runs with `mobCaptureDimensions=["minecraft:overworld"]`). The pickup drive proved **inventory exactly-once** twice and fixed two more live defects (tick suppression froze the projection's pickup delay forever — zeroed at adoption; restart re-adoption CAS-threw — `Runtime.adoptEntity` resolves the expected canonical state). Two of the three precise repros are now FIXED: **session reopen resumes from the store head (issue #34 closed 2026-07-23)** — external commits persist via `WorldStoreExternalHeads` + the quorum head via `WorldStoreVotePersistence.latestCommittedSnapshot`, `EntityLaneResume.resumeHead` picks the max-version head, and the live continuity series proved it (host reopen logged `Resuming region … from store head v287/v572/… (genesis would be v0)` for all 13 bound regions, zero errors, full `e2e-continuity.sh` PASS on the live display); **the clean-slate pickup vanish is FIXED and live-verified (issue #33 closed 2026-07-23)** — the skewed-signed-tick forward is no longer silently dropped (window bracketing, red-first `ActionForwardIT` case), pickups no longer cancel vanilla on the unconfirmed forward path (local-primary-only), and the NEW scripted drive `scripts/e2e-pickup.sh` passed live: fresh world, entity lane live, RCON-summoned 3×stone at the player's feet delivered **exactly once** (count pinned at 3 after settling, zero ERROR lines) via the lossless local-primary fallback; the committee-credit path is pinned headlessly by `CleanSlatePickupTest`. Both live scripts are foldable into CI via the new `e2e-live.yml` workflow (Xvfb, dispatch+nightly) — L-45 flips on its first green run. (The ghost-update CAS staleness is fixed on BOTH paths: `externalEntity` and — as of the live ownership drive, 2026-07-23 — `transferGhost`/`captureExternalTransfer` resolve their expected state from the canonical root (the bridge's cached prior is a hint, canonical is the authority; absent ids no-op, already-transferred ids replay-no-op), and `EntityCaptureBridge.onTickPost` crash-contains per-ghost lane failures so a guard mismatch can never take the server tick down — the un-contained form crashed the integrated server 9 s after opening on the first ownership run, and the fixed build passed the full O1–O4 series (per-player FOV drive, cross-owner visit, host kill → joiner re-host, host re-join; zero ERROR lines in all three client logs). Resume-from-head is a design increment: external commits are memory-applied and never reach the event log, so resuming requires persisting external deltas + a replay path on open.) Also remaining: pearl drives only — **5b digests landed 2026-07-23** (`SnapshotExtractor`: bit-complete per-section digests over all 4096 state ids with distinct empty/missing markers, wired into `WorldGenesisService.regionDigest`, pinned by `SnapshotExtractorTest` incl. the interior-edit-aliasing case) and **per-joiner identities landed 2026-07-23** (each member's own node key is its actor's signer on BOTH lanes — `registerActor` is now additive multi-key, any registered key admits, unregistered signers refused (`actorAdmitsUnderAnyRegisteredKeyAndRejectsUnregisteredSigners`); the interim session signer stays co-registered only until T16 retires the vanilla-session capture point) | T12 → T5/T9 | Reopen resumes from the store head; ghost captures refresh expected state from canonical; validated pickup credit lands exactly once on the vanish repro; 5b's extractor replaces coarse digests; per-joiner views/identities in the plan; pearl ghost/materialize/teleport drives. Headless exits already green: jqwik 3 replicas, disjoint committees, forced-process paired-log `@Invariant(11)`, bootstrap plan determinism, dirty-shutdown compensation, and 23,040 B/mob/min at 0 resync bps | RETIRING |
| L-46 | Task 31 GUI redesign is proven by view-model tests + compile-clean screens, but the presentation itself is unverified live: "Open to Nodera" taking the vanilla LAN slot, the single-player public-world badge's per-row placement (screen-level summary only for now — vanilla `WorldSelectionList` row geometry is not cross-package accessible), the tabbed `NoderaMultiplayerScreen`, and the `PieceMapWidget` green-fill grid all wait on a GUI env; ~~the feeds … defaulting empty~~ (**all four feeds live-wired as of 2026-07-23:** multiplayer worlds/trackers/rendezvous since the Task 31/33 fix, and the single-player world-list badge now reads `MultiplayerWorldFeed.ownWorldStatuses()` — the same worker STATE the multiplayer tab uses) | T31 | `runClient` (with L-45's harness) shows one "Open to Nodera" button in the LAN slot, a live public-world count on the world list, three working multiplayer tabs, and a piece-map that fills green as pieces arrive | OPEN |
| L-51 | **Password re-key pipeline (issue #37, follow-up to #36 F6).** **Pipeline landed headlessly 2026-07-24 (→ RETIRING):** a new additive `NODERA-REKEY` control verb (no protocol bump — SEED/GRANT precedent) carries the mod's freshly-packed archive path + the new plaintext password + the current signed `WorldIdentity`; the worker re-encrypts the archive under a fresh Argon2id salt (new key → new ciphertext → new `manifestRoot` + bumped version — reusing `WorldArchiveService.seedEncryptedArchive`, which had zero callers before), re-signs the identity with the new `manifestRef` (`WorldIdentity.resign`; authorship enforced by the signature — `worldId` is stable across re-keys), re-announces, and returns the re-signed identity. No old password is escrowed — the plaintext save on the author's own machine is the source (`WorldArchiver.packToSpool`). `NoderaHost.reconfigure` drives the Share-screen change: pack → `CompanionClient.rekey` → persist the re-signed identity + return (it deliberately does NOT re-`activate`, since that re-seeds a PLAINTEXT archive — live encryption at share is L-39's domain — which would supersede the re-key's encrypted manifest as newest and let joiners bypass the password); failure is surfaced honestly in chat and the world keeps its old password (never silent success). The crypto + identity round trip is pinned MC-free by `RekeyVerbIT` (re-encrypted blob decrypts under the NEW password only; wrong-author identity rejected); `CompanionClientRekeyTest` + `WorldArchiverPackToSpoolTest` cover the mod half. Remaining: the **live `runClient` exit** (Share screen → joiners must supply the new password) — the row retires on that. | #37 | `runClient`: author changes the password via the Share screen → the re-keyed world's new ciphertext replaces the old across seeders and joiners must supply the new password | RETIRING |
| L-52 | **No password gate on the live Minecraft join path (issue #36 "noted").** `NoderaHost.openGameServer` calls `publishServer(null, …)`; the world password protects only the archived content plane, not who connects to the live game. A listed world's tracker `mc/` route is public. (Operator access is still authenticated — via the key-checked `OperatorBridge` from issue #36 — but the *connection* itself is ungated.) | #36 | A password-protected world refuses a joiner at the live game server unless they supply the password | OPEN |
| L-54 | **Grants are not gossiped to non-hosting co-host peers (issue #36 "noted").** `WorldPermissionStore` persists grants on the author's host and `apply` re-verifies them, but a co-hosting peer does not yet receive grants over the mesh — its permission set is author-local until it fetches the world. Convergent AES-256-GCM content encryption also leaks plaintext equality (by design, in-code note). | #36 | A grant issued by the author reaches every co-hosting peer over the mesh and is applied identically | OPEN |
| L-55 | **Re-key does not evict the prior manifest version (issue #37 "noted").** A password re-key appends a new encrypted manifest version under the same `worldIdHex`; the old version's manifest + ciphertext blob stay in the worker's `WorldArchiveService.manifests` map and content store, `holdingsFor` advertises both, and the old blob remains decryptable with the OLD password (so a holder of the old password can still read the pre-re-key ciphertext). Correct end-to-end (the newest version is the re-keyed one, and joiners resolve to it), but orphaned ciphertext lingers until process exit. Seeder-side GC of superseded manifest versions is the follow-up. | #37 | A re-key supersedes the prior manifest version across seeders (old ciphertext evicted / unmaintained) | OPEN |
| L-56 | **Two connection settings can never be honoured as specified.** The companion app's Settings → Network exposes `max_connections_per_world` and `unlimited_connections_only`. Neither is implementable against the architecture as it stands, and both are *kept in the UI on purpose*, permanently badged with the worker's own reason rather than deleted — removing them would silently drop settings users already saved and would hide that the limitation is known. **Per-world connection caps:** `SocketPeerTransport` has no world dimension at all — a socket is not owned by a world, and the transport has no world concept to attribute one to; making it real needs either a world dimension on the transport or a world→manifestRoot accounting index, i.e. an architectural change, not a settings change. **Unlimited-connections-only:** no peer advertises a connection cap anywhere on the wire, so there is nothing to filter peers on; it would need a new field in the frozen membership family, mirrored in `nodera-codec`. `WorkerControlHandler` returns both under `rejected` with these reasons, and the app renders a distinct muted "not supported" badge — deliberately NOT the amber "not enforced yet", which implies the feature is coming. | T7 | Either the transport gains per-world attribution and a peer-advertised cap field, or both controls are removed from the UI with their saved values migrated | OPEN |
| L-57 | **Download rate limiting is request pacing, not shaping.** `network.max_download_bytes_per_sec` is enforced in `ContentTransferService.sendRequest` — the budget is checked *before* a piece is requested, because by the time bytes are received they have already crossed the wire. Piece sizes come from the manifest, so a byte budget converts to a per-window request budget with **±one-piece overshoot**; a 64 KiB piece can exceed a tight cap for one window. True TCP-level shaping is not possible without a custom transport, and the setting's hint says so rather than implying otherwise. | T7 | A sustained download honours the configured cap within one piece of overshoot, measured over a multi-window transfer | OPEN |
| L-58 | **Storage location and tracker endpoints apply only at worker restart.** `FsContentStore`'s root and the P2P bind port are `final`, and rebuilding their owners would drop live state (an open listener; every seeded manifest and holdings index). Nothing migrates existing blobs to a new directory either, so a live change would orphan what the node is already seeding. These keys are reported as `restart_required` and the app offers a restart button — but only when it supervises the worker; in attach mode it says to restart it where it was started, because the app must not kill a process it did not spawn. | T7 | Changing the archive directory relocates or re-indexes existing content without losing seeded state | OPEN |
