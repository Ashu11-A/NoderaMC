...
| L-47 | The Task 32 companion app is a scaffold: the Tauri shell (backend + React dashboard + tray + autostart) is workspace-excluded (Tauri native deps not in the headless gate) and the mod-side presence gate is headless-tested but enforcement defaults OFF; there is no bundled headless-peer jar feeding real metrics, no host/join control verbs, no per-OS installer, and no automated end-to-end installer + gate + cross-machine continuity test | T32 | A CI/tooling job builds the app, runs the gate both ways (present ⇒ start, absent ⇒ actionable abort), and proves a hosted world stays listed + joinable after the host closes Minecraft (the daemon kept it alive) | OPEN |
