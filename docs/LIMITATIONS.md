...
> **Burn-down pass (2026-07-25): L-10, L-11, L-15, L-18 RETIRED.** Four rows were carrying the
> RETIRING status while their *stated exit tests* were already green — the "remaining" notes on each
> listed follow-on scope (a chest GUI, a deposit debit, caves and biomes, rotation cadence) that the
> exit test never asked for. Each was re-verified against its own exit clause before moving, not
> against its prose: `OUTSIDE_GENERATED_TERRAIN` grep-verified absent from both languages (L-15);
> the container palette entries confirmed present and `UNSUPPORTED_PALETTE` confirmed to be the only
> palette gate, so container regions really are delegable (L-10); `PlayerInventoryTest`'s
> cross-region hand-off case confirmed to be the dupe-proof clause (L-11). **L-18's fourth clause —
> "Byzantine ITs green under adversarial peers" — was genuinely NOT met** and is closed here:
> `ByzantineWorkerTest` handed lying ballots straight to `CommitteeSession`, so no adversary had
> ever spoken the wire. The new `ByzantineMeshIT` puts a real adversarial peer on the mesh and the
> honest members run the production path against it.
>
> Deliberately **not** retired on this pass, with the reason: **L-25** — its exit reads "RuleSet SDK
> provides the legal async mutation API", and the SDK is L-21 (OPEN); `MutationGuard.verdictChecked`
> also still has no live mixin call site, so "the guard rejects the rest" holds headlessly only.
> **L-26** — its exit is "full redstone parity", and pressure plates (entity coupling) are still
> missing from the palette; the three named stages are green but the headline claim is not yet true.
