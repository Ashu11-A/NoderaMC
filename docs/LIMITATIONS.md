...
| L-51 | **Password re-key pipeline (issue #36 F6).** The `NODERA-PASSWORD` contract is fixed (it now carries the new password **plaintext** base64 over the loopback-only control socket — the old contract carried a hash, which provably cannot re-derive the Argon2id content key, and `reconfigure` silently proceeded as if it had re-keyed). `WorkerControlHandler.password` now verifies the worker is the world author and the verb never returns silent success — but the actual re-key (re-derive `WorldKeyMaterial` → re-encrypt + re-split the archive → new manifest version → `WorldIdentity.resign` → refresh announce) is **not wired**, so it declines with an honest, actionable error and `NoderaHost.reconfigure` surfaces the failure to the sharer in chat and re-shares with the unchanged password. Follow-up issue tracks the full pipeline. | #36 | `runClient`: author changes the password via the Share screen → the re-keyed world's new ciphertext replaces the old across seeders and joiners must supply the new password | OPEN |
