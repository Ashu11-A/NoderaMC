...
> (one task per Nodera module); the legacy→new mapping is [`Task.0.md`](Task.0.md) §4. The
> ordering/priority/difficulty analysis below remains valid — read it through the mapping.
...
world-list mixin / grant gossip / worker seeding documented in [`Task.33.md`](old/Task.33.md), new L-49) ·
...
— L-45) · MVP gate ([Task 7](old/Task.7.md)) proven
headlessly, not yet live · lane B complete to its headless/compile edge: Tasks 19–25 headless +
[Task 26](old/Task.26.md) view model + `Dist.CLIENT` screens (GUI pass pending) ·
...
| Class | Tasks |
|---|---|
| **Done (pure-Java scope)** | [2](old/Task.2.md) `core` · [3](old/Task.3.md) `simulation` |
| **Done (Rust services)** | [28](old/Task.28.md) standalone tracker — real binary driven from Java peers by `TrackerServiceIT`, embedded `TrackerService` deleted, L-44 RETIRED (mod-side announce scheduling rides the Task 26 live pass) |
| **Done (build architecture)** | [27](old/Task.27.md) monorepo + Rust workspace — `java/`+`rust/`+`fixtures/`, `nodera-codec` byte-exact against the Java golden frames, both toolchains gating CI |
| **Done (Rust services)** | [29](old/Task.29.md) standalone rendezvous + relay — `nodera-rendezvous` (registration/discovery/reservations/circuit-bridging/punch-coordination) + `java/transport-rendezvous` (direct-first / punch-upgrade / E2E-relay-fallback `PeerTransport`); `RendezvousRelayIT` drives the real binary; L-23 + L-27 RETIRED |
| **Shipped, GUI-env acceptance pending** | [1](old/Task.1.md) mod skeleton · [4](old/Task.4.md) protocol/transport (relay impl also pending) · [18](old/Task.18.md) diagnostics HUD (L-31 placeholders wait on 6/12) |
...
Derived from the [Task 0](old/Task.0.md) §1 dependency graph plus current state. Waves are sequential;
items inside a wave are parallelizable.

| Wave | Work | Why here |
|---|---|---|
| **0 — now, parallel** | (a) GUI-env acceptance passes for [1](old/Task.1.md)/[4](old/Task.4.md)/[18](old/Task.18.md)/[26](old/Task.26.md) (`runServer`/`runClient`); (b) ~~[19](old/Task.19.md)→[25](old/Task.25.md) headless + [26](old/Task.26.md) view model/screens~~ **done** — lane B is at its headless/compile edge; (c) keep growing 17\* scenarios; (d) ~~[27](old/Task.27.md) monorepo restructure~~ **done** — 28/29 unblocked | (a) is small and blocks every live milestone; (b) the torrent cluster is Minecraft-free and does NOT wait on waves 1–3; (c) standing; (d) one mechanical chore commit that unblocks the whole Rust lane (28/29) |
| **1 — the gate** | [5](old/Task.5.md) live: NeoForge capture mixins, snapshot streaming on a real server, 3-client soak, divergence burn-down | Phase 1 exit is the project's hard gate — live determinism evidence decides everything downstream |
| **2 — MVP** | [6](old/Task.6.md) live (capture/cancel + `ServerLevel` applier + 2-client run) → [7](old/Task.7.md) live 3-client quorum = **first playable milestone**; [11](old/Task.11.md) live half in parallel (~~headless guard/policy/monitor~~ **done** — mixins + tickets + fake-player detection remain) | 7 is the MVP gate; 11 is required before wave 3 runs on non-flat worlds |
| **3 — demotion prep** | [8](old/Task.8.md) live soak (needs 11 on real worlds); [9](old/Task.9.md) completion: RocksDB tier, chunk attachments, live forward-sync, continuity milestone | Phase 4 exit numbers + Phase 5 canonical state |
| **4 — network era** | ~~[28](old/Task.28.md) Rust tracker~~ ∥ ~~[29](old/Task.29.md) Rust rendezvous relay~~ **both landed** (Minecraft-free); [10](old/Task.10.md) gateway migration over the 29 transport (NAT reach); torrent chain ~~[20](old/Task.20.md)→[25](old/Task.25.md)~~ (headless); [26](old/Task.26.md) last (needs 19–25 + the 28 live feed) | 28/29 done after 27 (like lane B, they didn't wait on waves 1–3); 10's cross-NAT half now rides 29's shipped `transport-rendezvous`; live Task-25 adapters also need 7 |
| **5 — parity program** | [12](old/Task.12.md) → [13](old/Task.13.md) → [14](old/Task.14.md) → [15](old/Task.15.md) → [16](old/Task.16.md) | Burns `LIMITATIONS.md` §B to empty; 16 closes the ledger |
...
| Rank | Task | Why |
|---|---|---|
| P0 | [1](old/Task.1.md)/[4](old/Task.4.md)/[18](old/Task.18.md) acceptance remainders | Hours of work; gate every live milestone below |
| P0.5 | ~~[27](old/Task.27.md) monorepo + Rust foundation~~ | **Landed 2026-07-19** — layout moved, `nodera-codec` proven byte-exact against `fixtures/wire/`, both toolchains gate CI; 28/29 unblocked |
| 1 | [5](old/Task.5.md) live shadow validation | The determinism bet on real servers — everything gates on Phase 1 exit |
| 2 | [7](old/Task.7.md) committee MVP live | First playable milestone; the product exists once this passes |
| 3 | [6](old/Task.6.md) coordinator live | Direct prerequisite of 2 |
| 4 | [11](old/Task.11.md) interference guard (live half) | Headless guard/policy/monitor landed; the mixin choke point + tickets unblock every post-MVP phase on non-flat worlds |
| 5 | [9](old/Task.9.md) completion | Canonical certified state (Invariants 3–8); prerequisite for server demotion AND torrent value |
| 6 | ~~[19](old/Task.19.md) torrent data plane~~ | **Landed headless** — the foundation the rest of 19–26 builds on; only its mod-side consumers remain |
| 7 | [8](old/Task.8.md) live soak | Phase 4 exit (>90% committee-commit, honest CPU numbers) |
| 8 | 17\* debugger | Multiplies confidence of every lane; standing investment |
| 9 | ~~[29](old/Task.29.md) rendezvous relay~~ + [10](old/Task.10.md) migration over it | **29 landed** — `nodera-rendezvous` + `transport-rendezvous` retire L-23/L-27 (`RendezvousRelayIT` over the real binary); 10's cross-NAT migration rides this transport in the live pass |
| 10 | ~~[20](old/Task.20.md) tracker/multi-bootstrap~~ → ~~[28](old/Task.28.md) standalone Rust tracker~~ | **Landed 2026-07-19** — the always-on Rust service retired L-44: `TrackerServiceIT` lists a world by name, with its countdown and a DEAD verdict, after every Java seeder went silent |
...
| 18 | [26](old/Task.26.md) multiplayer GUI (live remainder) | View model + screens landed; the `runClient` pass + live tracker feed + create-world pipeline ship the feature to players |
| 19 | [14](old/Task.14.md) environment lane | Parity mid-game |
| 20 | [15](old/Task.15.md) deterministic mobs | Parity late-game; ghosts (12) are the working fallback meanwhile |
| 21 | [16](old/Task.16.md) trustless closure | Endgame; empties the ledger |
...
|---|---|---|
| 1 | [16](old/Task.16.md) | Several research-grade problems at once: BFT open membership, validated movement with prediction+rollback, deterministic worldgen, zero-reconnect local-replica view, mod SDK |
| 2 | [15](old/Task.15.md) | Deterministic mob AI + spawning inside vanilla's rate envelope; fixed-point pathfinding; per-species retirement |
| 3 | [14](old/Task.14.md) | Deterministic lighting is notoriously hard; fluids/fire/random-tick parity envelopes vs vanilla |
| 4 | [13](old/Task.13.md) | Redstone semantic fidelity + contraption ownership migration across regions |
| 5 | [5](old/Task.5.md) (live remainder) | Real-world divergence hunting — nondeterminism debugging on live servers is the project's riskiest grind |
| 6 | [12](old/Task.12.md) | Entity state in roots, ghost lanes, cross-region transfer without dupes |
| 7 | ~~[29](old/Task.29.md)~~ + [10](old/Task.10.md) (remainder) | **29 done** — signed-record registration, HMAC reservations, metered E2E relay circuits, punch coordination + `TransportSelector`, proven over the real binary; 10's migration UX under failure remains |
| 8 | [11](old/Task.11.md) (live remainder) | Wide, subtle surface: every foreign write source must reach the one mixin choke point, cheaply; the headless classification/conversion machinery is done |
| 9 | [9](old/Task.9.md) (remainder) | ~~RocksDB crash-consistency, replay-on-boot windows~~ done; live forward sync + committee-change certification remain |
| 12 | ~~[28](old/Task.28.md)~~ | **Done.** Tracker semantics are well understood (`docs/torrent/trackers.md`); the work is the announce family + cross-language conformance + ops hardening — and deleting the embedded Java tracker safely (`LEGACY.md`) |
| 13 | [8](old/Task.8.md) (remainder) | Real vanilla cross-region execution + live soak |
| 16 | [6](old/Task.6.md)/[7](old/Task.7.md) (remainders) | NeoForge wiring of already-proven pipelines |
| 18 | ~~[27](old/Task.27.md)~~ | **Done** — paths moved with history preserved (module names unchanged), codec port green against the golden fixtures on the first cross-language run |
| 19 | [26](old/Task.26.md) (live remainder) | Headless view model + compile-clean screens done; the fiddly part left is the GUI-env pass + live data feed |
| 20 | 17\*| Steady incremental scenario-writing, not deep |
| 21 | [1](old/Task.1.md)/[4](old/Task.4.md)/[18](old/Task.18.md) remainders | Acceptance passes in a GUI env |

---

## 5. Cross-cutting notes

- **Standing tasks.** 17\* (debugger) and [18](old/Task.18.md) (HUD) never "finish" in a
...
- **Three-lane staffing.** Lane A (Minecraft-facing): waves 1→2→3. Lane B (pure-Java): ~~19→20→21→22→
  23→24→25~~ → 26. Lane C (Rust services): [27](old/Task.27.md)→{[28](old/Task.28.md) ∥ [29](old/Task.29.md)} —
...
- **Monorepo rewrite queue.** Task 27 ([`MONOREPO.md`](old/Task.27.md)) moves every Gradle module
