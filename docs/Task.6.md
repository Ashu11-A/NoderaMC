...
| 6c | Host/join delegation + worker seeding: hosting rides the worker (announce loop, rendezvous registration, world-archive + region-piece seeding), grant gossip | 🚧 (**world-archive seeding landed 2026-07-23**: `WorldArchiveService` seeds the canonical save archive via `NODERA-SEED`, rides `ManifestHolding`s on every announce, answers `WorldManifestQuery` (tag 51/52), fetches any world's archive from the swarm via `NODERA-ARCHIVE`, and `STATE` reports real `maintained_pieces/bytes`; `WorldContinuityIT` proves host-death survival over the real tracker + rendezvous binaries. Remaining: validated-lane region-piece seeding (5b extraction) + grant gossip) | 5e (30c genesis/extraction), 2d (splitter reuse) |
