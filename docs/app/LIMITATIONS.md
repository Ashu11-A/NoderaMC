...
**Category:** app · **Last audit:** 2026-07-28 · Open or retiring rows: **2**
...
| L-91 | **The direct launch route cannot sign in.** Play assembles a full Minecraft command line — version-chain resolution through `inheritsFrom`, classpath ordering, natives extraction, the complete placeholder table, a probed Java runtime, `--quickPlayMultiplayer` — and none of it can run, because starting the game itself needs a Microsoft account token and obtaining one needs an Azure application id that **Microsoft approves per project**. No such credential exists in this repository and none can be produced from the source tree. The code path is built, unit-tested against fixtures, and gated on `NODERA_MSA_CLIENT_ID`; with the variable unset the app says which route it is taking instead, **before** it starts anything, rather than failing partway through a sign-in flow the player has already begun | [11](Task.11.md) | With `NODERA_MSA_CLIENT_ID` set to an approved id, Play spawns the JVM directly and the game opens in the world with no other launcher installed | OPEN |
| L-92 | **The offline account is for development only.** `launch::auth::Account::offline` fills the `${auth_*}` placeholders with a synthetic identity so command-line assembly can be exercised without a login. It is not a way to play: a LAN-opened vanilla world authenticates joiners against Mojang's session servers, so an offline client is refused by the ordinary Nodera target. It is reachable only from the direct route, which L-91 already gates | [11](Task.11.md) | Either a real token flow exists (L-91) and this stays test-only, or offline-mode targets are supported end to end and it is documented as a supported route | OPEN |
...
| L-94 | **The Android settings screens are thinner than the desktop's.** The native front end covers Home, Worlds, Activity, and Settings → Appearance / Battery / Privacy / About as real screens. Network and Storage render the core's answer rather than a form — the tracker list, the store subscriptions and the storage picker are readable but not yet editable from the phone. They are labelled as raw reads rather than dressed up as controls that do nothing | [11](Task.11.md) | Network and Storage on Android are editable, and the tracker-store add/preview/remove flow works from the phone | OPEN |
