...
| L-8 | Mob spawning cycles are vanilla-only. Deterministic spawn cycles landed: one attempt per 20 ticks over a randomly drawn owned column, actual standing cells collected from state, combined-light gate < 8, population cap 8 (interval × cap is the documented vanilla rate envelope), spawns entering the root as GHOSTs with deterministic ids and a 6000-tick horizon. A 2000-tick dark-shelter soak stays under cap, spawns only inside the shelter, and is replica-identical; a lit platform spawns zero. Remaining: passive species and per-species rates, mod-side mirroring of engine spawns, the vanilla spawn-suppression mixin, live evidence | [11](Task.11.md) | Deterministic spawn cycles (engine light + seeded RNG) match the vanilla rate envelope | RETIRING |
