...
| L-2 | Fluids excluded from the validated lane. Finite deterministic fluids landed: a per-cell automaton on the hashed scheduled-tick queue, level encoded in the block id, pure-neighbourhood desired state (fall first; horizontal contribution requires the neighbour to sit on solid; support loss drains the network), vanilla cadence and reach, deterministic water-over-lava, `BorderSignal.Kind.FLUID` with no halo writes. Fluid **interactions** landed (`FluidInteractionTest`, 6): a lava source water reaches becomes obsidian, a lava flow becomes cobblestone, and lava arriving above water becomes stone — water is never consumed, which is both vanilla's rule and what keeps the outcome a pure function of the neighbourhood. `RULES_VERSION` 4→5, palette literal `palette.v5`. **Remaining, stated precisely (audited 2026-07-26):** cross-region spread consumption is *not* a wiring change. A fluid reaching a region edge emits `BorderSignal.Kind.FLUID`, and nothing anywhere consumes it — a river stops dead at the boundary. It cannot simply be consumed, for two reasons the signal type states itself. First, a `BorderSignal` carries `kind, origin, target, tick` and **no state**, so the receiving region cannot compute the desired level at `target`: the cell driving the spread lives in the *sender's* region, outside the receiver's snapshot, and the engine deliberately never mutates halo. Second, border signals ride the execution result and **not the root**, so a receiver that acted on one would commit state derived from an input no certificate covers — which is the opposite of what the validated lane is for. What this needs is the source region's committed edge state reaching its neighbour as a certified input, which is a protocol addition. Also remaining: live evidence | [10](Task.10.md) | Water and lava spread deterministically across replicas, including cross-region via the migration lane | RETIRING |
