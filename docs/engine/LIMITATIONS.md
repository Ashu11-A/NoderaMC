...
| L-24 | The `mobCapture` ghost lane shipped **default-off** because nothing had been proven; every capture needed an explicit `entity.mobCaptureDimensions` opt-in, which is an all-species switch. That is no longer the state of the engine species-by-species: the zombie's behaviour originates in the validated lane — `MobAiRules`' seeded wander over the replicated root and `MobCombatRules`' validated vitals — so **`entity.mobCaptureSpecies` now defaults to `["minecraft:zombie"]`** and that species is captured anywhere, no dimension opt-in required. The dimension switch is kept and unchanged: it means "capture everything here", which is what a soak wants, while the species list means "this one is understood anywhere". The list grows as each further species' behaviour moves into the engine. Remaining: the live proof. **Half the exit is now green live**: `e2e-mobs.sh` **G2a passed** on a real dispatched run (30189534612) — a zombie summoned in a dimension that opted nothing in is captured on the species default alone, which is exactly this row's claim. The other half of the same exit clause is **G2b, which is red** and belongs to minecraft L-60: no revocation fires in the nether at all. This row cannot retire while the clause it shares is failing | [11](Task.11.md) | `e2e-mobs.sh` G2a: a zombie summoned in a dimension that opted **nothing** in is captured rather than revoked, and G2b shows a species the engine does not own still refuses its region | RETIRING |
