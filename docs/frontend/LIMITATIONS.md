...
| M-1 | **An arbitrary shared-storage folder may be picked but unusable.** Android 11+ withholds raw `File` access to shared storage regardless of the SAF grant. The worker no longer writes only with `java.nio.file`: `FsContentStore` writes through a `BlobDirectory` seam, `SafBlobDirectory` implements that seam over the app's `NoderaSafBlobs`, and a `content://` tree is accepted as `storage.peer_worlds_dir`, so the **archive lane** can live in a picked folder. Identity, the world registry and RocksDB deliberately stay in app-private storage — a signing key in a folder the user browses is a security regression, not a feature. **Remaining (2026-08-10): no handset.** Nothing here has touched a real `DocumentsContract`; the gate drives a JVM stand-in tree, and SAF's rename is ordered rather than atomic (content addressing is what keeps that from being a correctness problem — see `SafBlobDirectory`). Manual acceptance script: [`TESTING.md` §4.8](TESTING.md). | [13](Task.13.md) | A folder outside app-specific storage is picked and the worker writes a world archive into it | OPEN — worker half green on the gate (`ControlVerbsIT.ConfigVerbIT.aFolderPickedWithAndroidsFileManagerBecomesTheArchive`, `SafBlobDirectoryTest`, `FsContentStoreBlobDirectoryTest`); the exit test names a device and no device has run it |
