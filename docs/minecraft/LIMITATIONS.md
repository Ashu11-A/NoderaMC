...
| L-59 | **A password-protected world was published in the clear.** Found live by the re-key suite: content is encrypted at share and at every re-key, but the *continuous* streaming lane kept seeding PLAINTEXT archives on its own cadence — each one a newer version than the ciphertext, so the newest thing on the network for a "password-protected" world was its unencrypted save. The refresh now goes through the same re-encrypting path the re-key uses (fresh salt, new ciphertext, new manifest root, re-signed identity, re-announce, superseded versions evicted) and has **no plaintext fallback**: if encryption fails, the network keeps the last good ciphertext and the log says so | [5](Task.5.md) | `e2e-rekey.sh` R2b: with a fast streaming cadence, every archive the worker seeds for a password-protected world is ciphertext — no plaintext version is ever published | RETIRING |
