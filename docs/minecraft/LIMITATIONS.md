...
| L-60 | **Ghost capture and revocation ran only on the node whose lane owned the region.** `EntityCaptureBridge.captureJoin` acted only when `runtime.delegated(region)` — the *server's* lane — and under field-of-view ownership the plan hands every region to the PLAYERS' nodes ("no regions fall to this node" is in every dedicated-server log), so the one node that could see a non-delegable entity was the one node forbidden from acting on it. **Fixed:** the dimension opt-in is a config fact true on every node, so it is now evaluated BEFORE the ownership gate, and the observer announces a `RegionRefusal` (tag 61) that makes the owning lanes drop the region; a refused region is never re-activated. `RegionRefusalIT` proves it out of game — an observer holding **no** replica stops the region on both seats, repeats do not re-announce, neighbours are untouched. Remaining: the live run (`e2e-mobs.sh` G2 now asserts instead of skipping). The capture half is unchanged and stays with its owner, which is correct | [2](Task.2.md) | `e2e-mobs.sh` G2 asserts rather than skips: a non-delegable entity in an un-opted-in dimension revokes its region, names the reason, announces the refusal to the mesh, and the session plays on | RETIRING |
| L-80 | **Block capture only runs on the node whose lane holds the region.** The live capture lane exists — `BlockCaptureBridge` turns a real place or break into a signed `PlaceBlockAction`/`BreakBlockAction` on the same submit path as entity actions, with the palette binding (`VanillaPalette` + `PaletteMapper`) deciding what is consensus state — but its first gate is `delegated(region)`. On a **player-hosted** world that is exactly right: the host's node holds the regions its player views, and its edits are captured. On a **dedicated** server the field-of-view planner hands every region to the players' nodes, so the one process that sees the block events holds no seat and captures nothing. This is L-60's fault line for blocks; the fix is the same shape — an observer that forwards to the region's primary without holding a replica, which needs the membership-side seat lookup of [#45](https://github.com/Ashu11-A/NoderaMC/issues/45) rather than the local-replica lookup `forwardToPrimary` uses today | [2](Task.2.md) | On a dedicated server with all regions owned by player nodes, a player's place and break reach the owning node's committee and commit: `/nodera debug capture` reports captured edits rather than `REGION_NOT_DELEGATED`, and the resulting delta appears in the region's chain | OPEN |
