...
| L-45 | **No automated real-client acceptance in CI.** Both halves are built and both have passed by hand: dev runs launch from Gradle on the reconciled NeoForge pin, and eight scripted suites drive real clients against the real tracker and rendezvous binaries — the continuity series passed all seven stages on a live display (share → listing → second client in-world → 11.2 MB archive on the network → host killed → joiner recovered, re-opened, and re-hosted in **3 seconds**). En route the series caught four real defects that no headless test could have. The suites are folded into CI under Xvfb (`.github/workflows/e2e-live.yml`, three jobs); its first runs found a fifth defect — a game dir with no `options.txt` puts the accessibility onboarding screen in front of quick play, so every scripted client sat on it for the whole timeout — and the launcher now seeds first-run options and guards the join waits on the stalled screen. What remains is a green run of that workflow | [1](Task.1.md) | The suites' stages pass end to end in CI: share → a second client joins through tracker and rendezvous → host killed → the joiner recovers the world from the network | RETIRING |
...
| L-52 | **The live join path is gated on the world password.** The gate is a configuration-phase challenge-response: the host derives a gate key from the password once (random salt, the content plane's KDF and cost), each connection gets a fresh nonce, and the joiner answers `HMAC(gateKey, ctx ‖ nonce ‖ worldId)` before a player is ever created — a refusal costs the joiner a connection, not a tick of gameplay. The password itself never crosses the wire, a proof is bound to its nonce **and** its world, an arming failure **seals** the world shut rather than silently opening it, and a dedicated server (which has no Share screen) sets it with `host.sharePassword`. Proven headlessly by `JoinPasswordGateTest` + `HostJoinGateTest`; the live half is the new `e2e-password.sh` suite, whose second stage is the negative control — the same client, with the password, joins normally | [5](Task.5.md) | A password-protected world refuses a joiner at the live game server unless they supply the password | RETIRING |
