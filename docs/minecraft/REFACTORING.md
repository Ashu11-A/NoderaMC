...
| [`…/common/CompanionGateTest.java`](../../peer/src/test/java/dev/nodera/peer/control/CompanionGateTest.java) | 117 | 13.7 | storage/transport fixtures | Shared loopback-socket test base. |
...
| [`…/common/NoderaConfig.java`](../../endpoints/neoforge-mod/src/main/java/dev/nodera/mod/common/NoderaConfig.java) | 290 | 2.8 | self (141-148↔205-211) | Two duplicated config-validation blocks (rendezvous vs tracker route parse) — already split on purpose, but the 8-line run could share a validator. **Reviewed 2026-08-05 and deliberately left**: the row's own "already split on purpose" is the answer. A tracker route accepts a `tcp://`/`udp://` scheme and a rendezvous relay accepts neither, because a relay circuit is a long-lived stream with no UDP surface. A shared validator would have to take the difference as a parameter, which is the split again with one more indirection. |
...
0. **Reduction pass, 2026-08-05 (issue #213 phase 4).** The two god-class splits below are pure
   ...
   the next candidate; they were left because the shared part is a `StreamCodec` + `TYPE` pair whose
   generic signature is per-record, so a base costs about what it saves.
