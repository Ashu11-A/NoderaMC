...
| `e2e-commands.sh` | Two players × every `/nodera` command with response validation, the live extraction drive (four unsupported blocks placed, the extractor's palette-exclusion count must move by exactly four), plus the in-game self-test tree walk and benchmark | ~9 min |
