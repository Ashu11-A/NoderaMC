...
| `peer.PeerShutdownHook` | peer | **Blocked on a piece-push lane, not on a call site** | A shutdown hook nothing installs is a shutdown that does not happen — but installing it needs `EmergencyFlush`, and `EmergencyFlush.PieceTransfer` is a *push*: "take this piece and acknowledge you stored it". Production has no such path. Content moves by pull (`ContentTransferService`), and the two messages that would carry a push, `ArchiveReplicaAssignment`/`Ack` (tags 30/31), are in the dead-tag table above precisely because their sender was never built. `LocalHolding` also requires the whole contiguous blob in memory and re-verifies every piece hash on construction, which is not a thing to do at shutdown for a multi-GiB archive. Task 24 scope: build the push lane, or delete the hook and the flush together |
