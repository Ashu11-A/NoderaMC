...
| `protocol.wire.MessageRouter`, `protocol.wire.CorrelationTable` | transport | **Decide: wire or delete** | Verified: `MessageRouter` has zero non-test, non-self references. Request/response correlation is a stated NDR2 capability, so the frame's `correlationId` is written by every send and consumed by nothing. Of the router's three declared columns, **authorisation is now enforced without it** (2026-07-30, at `PeerRuntime.dispatch`) because that half was a security property and could not wait for a god-class split. What is still only in the router is per-kind `HandlerMode` (the worker still fans one application lane out to six services by hand, last-wins) and the `UnknownKind` → `Nack` answer, so an unsupported kind is still a silent drop the sender can only diagnose as a timeout. Wiring the rest is the `PeerRuntime` split in the sequencing list below |
