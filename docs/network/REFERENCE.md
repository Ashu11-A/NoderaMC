...
| `NODERA-PROGRESS <verified> <total> [stagedPathB64]` | Written by the worker *during* a fetch, before the terminal line. The timeout was being spent as a stall budget by the worker and as a wall clock by the client, so a healthy transfer expired underneath itself. The third token is additive and appears on `NODERA-FETCH-REGION` only: it names a file holding the region's columns verified so far, so the caller can draw arriving terrain (network L-33). A client that only knows the two-token form ignores it, which that line has always required |
...
| `NODERA-FETCH-REGION <ver> <worldId> <dim> <x> <z> <destPathB64> [haveRoot] [timeout]` | The mirror of the above, and the direction that did not exist: regions were seeded, hashed and announced, and nothing ever downloaded one. Since 2026-08-10 it stages the columns that have arrived as it goes, on `NODERA-PROGRESS` lines, instead of answering once at the end |
