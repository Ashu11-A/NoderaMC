...
**Category:** network · **Last run:** 2026-07-30 · **996 Java test cases + 73 Rust (`nodera-codec`)
`#[test]` · 0 failing at the last gate** — Java counts come from Gradle XML reports and sum the module
table below (187 + 157 + 652). Worker-module cases are accounted under the worker category.
...
| `peer` | Distribution, runtime, discovery, archival, diagnostics, validation lane, durable coordinator state, the endpoint tenant boundary, the L-16 prediction feed, and the NDR2 authorisation table **as the runtime applies it** (`SenderAuthorisationIsEnforcedTest` — a forged goodbye cannot evict, a forged join cannot enrol) | 652 | 🚧 |
