...
**Category:** worker · **Last audit:** 2026-07-29 · Open or retiring rows: **4**
...
Four rows, all owned by [Task 8](Task.8.md). The category's previous staged row, L-41, retired on
...
same date with its exit tests green. **W-DUP-3, the last row of that bundle, retired on 2026-07-29**
— the launched worker distribution was driven through boot, a registry write, and a restart with its
identity and registry on a real FAT32 mount. All four remaining rows are RETIRING on a **live run**,
not on missing code.
...
- When a design choice trades against the §B row, prefer the choice that keeps the exit test
  achievable.
- Two rows (W-REPL-2, W-REPL-3) retired on 2026-07-28 and W-DUP-3 on 2026-07-29; if any of them
  regresses, open a **new** bug row here rather than resurrecting the old id.
- Every remaining row is RETIRING on a **live** clause. None of them is waiting for code, so a pass
  that adds more code to them is almost certainly the wrong pass — what they need is a run.
