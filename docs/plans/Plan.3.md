...
| engine L-2 | **needs a protocol addition + live** | Fluid interactions landed 2026-07-26. The earlier note here said cross-region spread "can start today" as engine work; that was **wrong**, and reading `BorderSignal` says why. The signal carries `kind, origin, target, tick` and no state, so the receiving region cannot compute the desired level at `target` — the driving cell is in the sender's region, outside the receiver's snapshot, and the engine deliberately never mutates halo. And border signals ride the execution result, **not the root**, so acting on one commits state no certificate covers. The fix is the source region's committed edge state reaching its neighbour as a certified input — the same shape as minecraft L-50's pearl half, and the same answer: a protocol addition, not more wiring. |
