...
<!-- AI-AGENT-INSTRUCTION: Reservations are the abuse boundary of this service: no reservation, no
     ...
     Context: the standalone Rust rendezvous+relay binary. Sub-deliverables 1-9 all ✅; 71 Rust
     tests in the crate. Key files: service.rs:192 (handle_frame dispatch), reservation.rs:39
     (ReservationKeeper HMAC proof), circuit.rs:56 (CircuitMeter per-direction accounting),
...
Complete. **71 Rust tests** in `nodera-rendezvous` (62 `#[test]` + 9 `#[tokio::test]`, the latter
the real-socket suite in `wire.rs`). `RendezvousRelayIT` spawns the **real binary** and drives two
relay-only Java peers through it: they register, discover each other, and a `PeerJoin` plus a
keep-alive cross the end-to-end-encrypted circuit **byte-exact**; exhausting the reservation's byte
ceiling tears the circuit down with the right reason code; and the selector reports the direct path
when one is allowed.
...
| 5 | `observed.rs` — the caller's reflexive address | ✅ (emitted inline in `service.rs::on_register` as `ObservedAddress`) |
...
6. ✅ `cargo test` green (71) and conformance green.
