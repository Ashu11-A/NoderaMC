...
**Category:** server · **Last audit:** 2026-07-26 · Open or retiring rows: **10**
...
|---|---|---|---|---|
| L-61 | **The plugin now exists, for what task 1 covers.** `java/paper-plugin` builds `nodera-endpoint.jar` (a Paper plugin descriptor with `folia-supported: true`, `core` shaded in), and `scripts/e2e-endpoint.sh` **passes against a real Paper 1.21.1**: the plugin enables, names its platform from the presence of the regionised scheduler rather than from a version string (a fork renames itself; the scheduler is the load-bearing difference), reports ALIGN-1 as not applicable on a single-threaded platform, and disables cleanly with no exception in the log. ALIGN-1 itself is `dev.nodera.core.region.RegionAlignment` — Minecraft-free and tested exhaustively over ±64 regions × exponents 3–6, because it is arithmetic that decides whether two threads can write one authority unit. Remaining: `e2e-folia.sh` and `e2e-plugins.sh` (the row previously claimed all three suites were committed; only the harness library was), a staged Folia build to run the refusal path against, and everything the plugin does not do yet — it hosts no peer, which is task 2 | [1](Task.1.md) | All three suites get past the jar preflight: one `nodera-endpoint.jar` enables cleanly on Paper **and** Folia, logs its platform, and passes the ALIGN-1 preflight | RETIRING |
...
| L-66 | **The version pin is unresolved.** The mod pins Minecraft 1.21.1 / NeoForge 21.1.238 / Java 21; the Folia study in [`../minecraft/folia/`](../minecraft/folia/) documents Folia targeting Minecraft 26.1.2 / Java 25. A 1.21.1 client cannot join a 26.1.2 server, so the mixed-client suites need Paper **and** Folia builds of the mod's own Minecraft version, and no such build has been resolved yet | [1](Task.1.md) | `e2e-endpoint.sh` and `e2e-folia.sh` are both green against pinned Paper and Folia builds of the Minecraft version the mod pins, resolved through the PaperMC v2 API in CI | OPEN |
