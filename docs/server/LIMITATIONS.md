...
|---|---|---|---|---|
| L-61 | **The plugin does not exist.** `java/paper-plugin` is unwritten; `scripts/e2e-endpoint.sh`, `e2e-folia.sh`, and `e2e-plugins.sh` are committed and report `SKIPPED (server task 1 — no plugin jar)` at their preflight, exiting 0 so a nightly run stays honest rather than red-for-unbuilt | [1](Task.1.md) | All three suites get past the jar preflight: one `nodera-endpoint.jar` enables cleanly on Paper **and** Folia, logs its platform, and passes the ALIGN-1 preflight | OPEN |
...
| L-63 | **`ViewOwnershipPlanner` takes one `PlayerView` per node**, so an endpoint with forty players could contribute at most one disc to the ownership plan — the other thirty-nine would be invisible to it and their regions would go unowned or fall to whoever happened to be nearest | [3](Task.3.md) | `ViewOwnershipPlannerTest` covers a node holding N views with the min-distance rule, and two independent peers compute an **identical** plan for a 20-tenant endpoint | OPEN |
