...
| `EntityTickEvent.Pre` **cancel** | projection pinning: `ticksLived` reset, velocity zeroed, `pickupDelay` held at vanilla's 32767 "never" sentinel, mob pickup denied, re-anchored past a 0.5-block hysteresis band | **approximation** | Bukkit cannot cancel a tick — [L-69](LIMITATIONS.md). Built 2026-08-10 as `endpoint.paper.entity.ProjectionPinner`, dispatched on each region's own `RegionScheduler`. The band rather than a per-tick position write: the platform integrates the entity **before** the region task runs, so zeroing velocity cannot undo the tick that already happened, and writing the position back every tick is client-visible jitter |
...
| `ItemEntityPickupEvent.Pre` | `EntityPickupItemEvent` / `PlayerAttemptPickupItemEvent` | exact for an unvalidated item; for a **pinned** one the event **cancels** and the region tick routes the intent | For a validated item the event cannot be the credit path: it does not fire while `pickupDelay > 0`, which the pin holds at 32767. So the pin detects a taker within one block on the region tick and the lane credits; the event handler's job is to stop vanilla crediting behind it if another plugin zeroes the delay. Exactly-once is a property of `ProjectionPinner.claim`, not of event ordering |
...
| `LevelTicksMixin` (scheduled-tick suppression) | reconcile through the interference guard | **approximation** | no Bukkit twin at all — [L-67](LIMITATIONS.md). Measured 2026-08-10: one two-tick repeater clock is 600 foreign writes per revocation window here and **zero** on the mod, against a bound of 60, so the approximation currently costs the region its delegation. The certificate rate is still bounded, by the commit cadence rather than the write volume ([Task.5.md](Task.5.md) §Design) |
