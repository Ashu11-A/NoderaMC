...
|---|---|---|---|---|
| T-1 | OPEN | The converted scenarios have not all been executed live end to end since the conversion. The classes carry the same stages, evidence strings and timeouts as the shell suites they replace, and the tool builds and enumerates them, but a green nightly across the whole matrix is what turns "converted" into "proved". **Three preconditions of this row landed 2026-08-10, and none was visible from the row's own text:** the workflow had produced no matrix at all since 2026-07-30 — twelve consecutive nightlies died in the plan job for want of a JDK 21, in about ninety seconds, having booted no suite; when it last did produce one, "the full matrix" was thirteen of the nineteen unattended scenarios, because the workflow kept its own list of ids beside the check that validated it ([#142](https://github.com/Ashu11-A/NoderaMC/issues/142)); and the harness bound the product's own ports, so on any machine with Nodera installed *every* scenario was refused before its first stage ([#266](https://github.com/Ashu11-A/NoderaMC/issues/266), and the 0-passed/17-failed report of 2026-08-07, which recorded that bug and not any scenario's behaviour). The plan job now installs Temurin 21 and derives the matrix from `nodera-test list --ids --exclude-tag hardware`; the harness runs on its own 26500+ block. What remains is exactly what the row always asked for: a nightly that is **green**, over all nineteen — and T-6 says the reference box cannot host the client-bearing half of it. | [testing 1](Task.1.md) | a green `e2e-live` run over the full matrix |
