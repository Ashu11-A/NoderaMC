...
Whole-tree Java total for that same run: **2,273 tests · 0 failed**. Twelve real-binary ITs skipped
in that *local* run because the Rust service binaries were not built in the same tree; the `java` job
builds them, which is what its `Nothing skipped into green` step exists to enforce. That figure is the
