...
class RocksCrashRecoveryIT {
    ...
    void forciblyKilledWriterReopensCleanWithAnIntactChain() throws Exception {
        ...
        try (RocksWorldStore recovered = RocksWorldStore.open(
                dir, StoreFixtures.GENESIS, StoreFixtures.HASHES, false)) {
            long lastId = recovered.events().lastEventId(StoreFixtures.REGION);
            assertThat(lastId).isGreaterThanOrEqualTo(0); // READY was printed after event 0

            List<CommittedEventEnvelope> log = recovered.events().readFrom(StoreFixtures.REGION, 0);
            assertThat(log).hasSize((int) lastId + 1);
            for (int i = 0; i < log.size(); i++) {
                CommittedEventEnvelope event = log.get(i);
                assertThat(event.eventId()).isEqualTo(i);
                assertThat(event.prevRoot()).isEqualTo(StoreFixtures.chainRoot(i - 1));
                assertThat(event.resultingRoot()).isEqualTo(StoreFixtures.chainRoot(i));
            }
            assertThat(recovered.events().headRoot(StoreFixtures.REGION))
                    .contains(StoreFixtures.chainRoot(lastId));

            // The recovered store is live: the next chained append lands on the recovered head.
            recovered.events().append(StoreFixtures.chainedEvent(StoreFixtures.REGION, lastId + 1));
            assertThat(recovered.events().lastEventId(StoreFixtures.REGION)).isEqualTo(lastId + 1);
        }
    }
    ...
    public static final class WriteStormMain {
        ...
        public static void main(String[] args) {
            ...
            RocksWorldStore store = RocksWorldStore.open(
                    dir, StoreFixtures.GENESIS, StoreFixtures.HASHES, false);
            long id = 0;
            store.events().append(StoreFixtures.chainedEvent(StoreFixtures.REGION, id++));
