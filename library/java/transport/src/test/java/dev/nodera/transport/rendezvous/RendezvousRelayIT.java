 ...
 * Java-only checkout stays green. A run that <i>has</i> built it says so with
...
final class RendezvousRelayIT {
    ...
    private static final String BINARY = "nodera-rendezvous";

    private SpawnedService service;
    ...
    void twoRelayedPeersExchangeEndToEndTrafficAndTheByteCapTearsDown() throws Exception {
        Optional<Path> binary = SpawnedService.binary(BINARY);
        assumeThat(binary).as(SpawnedService.buildHint(BINARY)).isPresent();
        RendezvousEndpoint endpoint = startService(binary.orElseThrow(), 65_536);
    ...
    private RendezvousEndpoint startService(Path binary, long maxBytes) throws IOException {
        service = SpawnedService.start(binary, """
                ...
                """.formatted(maxBytes));
        return RendezvousEndpoint.parse(service.endpoint());
    }
