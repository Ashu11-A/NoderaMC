//! Build script for the companion app's core.
//!
//! This resolves the project's published service index — the list of trackers and relays the app
//! ships with as its built-in store — and writes it into `OUT_DIR` for `stores.rs` to `include_str!`.
//!
//! It used to live beside `tauri_build::build()` in the app crate. The two jobs were never related,
//! and only one of them belongs to code that must build without a webview.
//!
