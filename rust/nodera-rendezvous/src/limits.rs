...
/// A fixed-window request counter keyed by source IP.
#[derive(Debug)]
...
impl RequestQuota {
    /// A quota of `limit` requests per `window_millis`.
    ...
    /// Count one request from `source`; returns whether it is within the quota.
...
mod tests {
    ...
    #[test]
    fn requests_within_the_limit_pass_and_the_next_is_refused() {
        let mut quota = RequestQuota::new(1_000, 3);
        for _ in 0..3 {
    ...
    fn the_window_resets() {
        let mut quota = RequestQuota::new(1_000, 1);
        assert!(quota.admit(ip(1), 0));
    ...
    fn sweeping_drops_idle_counters() {
        let mut quota = RequestQuota::new(1_000, 1);
        quota.admit(ip(1), 0);
        assert_eq!(quota.tracked_sources(), 1);
        quota.sweep(5_000);
    ...
    fn a_zero_limit_disables_the_quota() {
        let mut quota = RequestQuota::new(1_000, 0);
        for _ in 0..1_000 {
            assert!(quota.admit(ip(1), 0));
        }
    }
