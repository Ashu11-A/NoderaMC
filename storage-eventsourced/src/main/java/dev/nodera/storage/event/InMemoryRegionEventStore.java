...
public final class InMemoryRegionEventStore implements RegionEventStore {
    ...
    private final Map<RegionId, List<CommittedEventEnvelope>> logs = new TreeMap<>(REGION_ORDER);
    private final Map<RegionId, StateRoot> heads = new TreeMap<>(REGION_ORDER);
    ...
    public void append(CommittedEventEnvelope event) {
        ...
        long expectedId = log.isEmpty() ? 0L : log.get(log.size() - 1).eventId() + 1L;
        ...
        if (!log.isEmpty()) {
            StateRoot head = heads.get(event.region());
            if (!event.prevRoot().equals(head)) {
