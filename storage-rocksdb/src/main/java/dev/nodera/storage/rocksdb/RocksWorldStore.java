...
public final class RocksWorldStore implements WorldStore, AutoCloseable {
    ...
    private final class RocksRegionEventStore implements RegionEventStore {
        ...
        public void append(CommittedEventEnvelope event) {
            ...
            long last = lastIds.getOrDefault(region, -1L);
            long expectedId = last + 1L;
            if (event.eventId() != expectedId) {
                throw new IllegalStateException("non-monotonic eventId for " + region
                        + ": expected " + expectedId + " got " + event.eventId());
            }
            if (last >= 0 && !event.prevRoot().equals(heads.get(region))) {
                throw new IllegalStateException("broken event chain for " + region
